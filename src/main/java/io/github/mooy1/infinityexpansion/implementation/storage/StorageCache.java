...
final class StorageCache {
    ...
    private static final String VOID_EXCESS_TRUE = ChatColors.color("&7已满:&e 是");
    private static final String VOID_EXCESS_FALSE = ChatColors.color("&7已满:&e 否");
        ...
        if (this.menu.hasViewer()) {
            if (this.amount == 0) {
                ...
                this.menu.replaceExistingItem(STATUS_SLOT, new CustomItem(
                        Material.CYAN_STAINED_GLASS_PANE,
                        "&b状态",
                        "&6已储存: &e" + LorePreset.format(this.amount) + (this.unit.displayMax ? this.unit.maxString + (100 * this.amount) / this.unit.max + "%)" : ""),
                        this.voidExcess ? VOID_EXCESS_TRUE : VOID_EXCESS_FALSE,
                        "&7(点击开关)"
                ), false);
            }
