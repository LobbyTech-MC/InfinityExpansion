...
public final class StoneworksFactory extends AbstractMachineBlock implements RecipeDisplayItem, NotHopperable {
    ...
    @AllArgsConstructor
    private enum Choice {
        NONE(new CustomItemStack(Material.BARRIER, "&c空", "", "&7 > 点击切换"),
                new Material[0],
                new Material[0]
        ),
        FURNACE(new CustomItemStack(Material.FURNACE, "&8烧制中", "", "&7 > 点击切换"),
                new Material[] { Material.COBBLESTONE, Material.SAND },
                new Material[] { Material.STONE, Material.GLASS }
        ),
        CRUSH(new CustomItemStack(Material.DIAMOND_PICKAXE, "&8粉碎", "", "&7 > 点击切换"),
        ...
        Choice(CustomItemStack item, Material[] inputs, Material[] outputs) {
			this.item = item;
			this.inputs = inputs;
